...
class Cal_CZerror_byNoise:
    ...
    def __init__(self,
        ...
        if is_psd_in_chip:
            self.kv_c_20M = 58997285148.46638/np.sqrt(10**(-32/10))  #partial(f)/partial(V)  coupler在20M耦合处的频率随电压变化的斜率
            self.kv_c_off = 29950204279.18776/np.sqrt(10**(-32/10))  #partial(f)/partial(V)  coupler在关断点处的频率随电压变化的斜率
            self.kv_q_300M = 3036969283.1584916/np.sqrt(10**(-39/10)) #partial(f)/partial(V)  qubit 在偏置300M处的频率随电压变化的斜率
        else:
            self.kv_c_20M = 58997285148.46638  #partial(f)/partial(V)  coupler在20M耦合处的频率随电压变化的斜率
            self.kv_c_off = 29950204279.18776  #partial(f)/partial(V)  coupler在关断点处的频率随电压变化的斜率
            self.kv_q_300M = 3036969283.1584916 #partial(f)/partial(V)  qubit 在偏置300M处的频率随电压变化的斜率
        ...
        (PN_q,Interg_q,T2s_q ,fit_datas_q) = self.Cal_T2_by_Sw(Sw_func = self.PSD_omega_q300M,
                                                         omega_s = 0.01*2*np.pi,
                                                         omega_e = 50e6*2*np.pi,
                                                         int_limit = 5e3,
                                                         tau_list = self.tau_list,)
        (PN_20M,Interg_20M,T2s_20M ,fit_datas_20M) = self.Cal_T2_by_Sw(Sw_func = self.PSD_omega_c20M,
                                                         omega_s = 0.01*2*np.pi,
                                                         omega_e = 50e6*2*np.pi,
                                                         int_limit = 5e3,
                                                         tau_list = self.tau_list,)
        ...
        #                                                  )
        (PN_geff,Interg_geff,T2s_geff,fit_datas_geff) = self.Cal_T2_by_Sw(Sw_func = self.PSD_XYg_c20M,
                                                         omega_s = 0.01*2*np.pi,
                                                         omega_e = 50e6*2*np.pi,
                                                         int_limit = 5e3,
                                                         tau_list = self.tau_list,)
        ...
        self.T_gate_CZ = 100e-9    #CZ门时间100ns
        self.error_T2s_CZ = 1-np.exp(
            -2*(self.T_gate_CZ/self.T2s_q)**2
            - (2*self.T_gate_CZ/self.T2s_20M+np.sqrt(2)*self.T_gate_CZ/self.T2s_geff)**2
            # - 3*(self.T_gate_CZ/self.T2s_off)**2
        )
        
        print(f"""
              =========================================
              The expected CZ-gate error is: {np.round(self.error_T2s_CZ*100,4)}% ! 
              The target error should to be controlled below 0.04%.
              =========================================
              """)
...
"""
CZerror_Caled = Cal_CZerror_byNoise(Freq_r, PSD_dBm, is_psd_in_chip = False,
                                    tau_list = tau_list)
...
plt.plot(t_list/1e-6,CZerror_Caled.fit_Tp_func(t_list,*CZerror_Caled.fit_datas_q), 'r--',
...
plt.plot(t_list/1e-6,CZerror_Caled.fit_Tp_func(t_list,*CZerror_Caled.fit_datas_20M),'b--',
