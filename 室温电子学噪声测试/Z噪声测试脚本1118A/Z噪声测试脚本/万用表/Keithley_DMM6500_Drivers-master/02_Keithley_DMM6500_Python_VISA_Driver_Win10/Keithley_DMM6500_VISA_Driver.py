...
class DMM6500:
    ...
    def Measure(self, count):
        ...
        return self.SendCmd(sndBuffer)


    """添加函数功能 2023/09/04"""
    
    def Measure_repeat(self, count):
        """ DMM measure repead with defined count """
        
        sndMcount = "dmm.measure.count = {}".format(count)
        self.SendCmd(sndMcount)
        
        self.QueryCmd("print(dmm.measure.read())")
        # sndBuffer = self.QueryCmd("print(defbuffer1.n)")[0:-1]
        # sndBuffer = "printbuffer({}, {}, defbuffer1)".format(1, count)
        return 
    
    def Get_Buffer_Data(self, startIndex = 1, endIndex = None,
                        buffer_i = "buffer1"):
        
        buffer_num = int(self.QueryCmd("print(def{}.n)".format(buffer_i)))
        endIndex = buffer_num if not endIndex else endIndex
        sndBuffer = "printbuffer({}, {}, def{})".format(startIndex, endIndex,buffer_i)
        return self.QueryCmd(sndBuffer)
    
    def Clear_Buffer(self,buffer_i = "buffer1"):
        sndBuffer = "def{}.clear()".format(buffer_i)
        self.SendCmd(sndBuffer)
        return                
