...
"""配置测量参数"""
frequency_to_measure = 1000  # 要测试的信号频率，单位是 Hz
measurement_time = 60*3   # 测量时间，单位是秒
...
#设置采集频率范围
M_freq_list = np.linspace(5**0.2,500e3**0.2,30)
...
for ii in range(len(M_freq_list)):
    ...
    if M_freq_list[ii]<50:
        sr860.write("SCAL 17")  # 设置灵敏度范围，16 代表 2 uV rms 全量程
    # elif 10<=M_freq_list[ii]<50:
    #     sr860.write("SCAL 17")  # 设置灵敏度范围，17 代表 2 uV rms 全量程
    elif 50<=M_freq_list[ii]<500:
        sr860.write("SCAL 18")  # 设置灵敏度范围，18 代表 1 uV rms 全量程
    elif 500<=M_freq_list[ii]<3000:
        sr860.write("SCAL 19")  # 设置灵敏度范围，19 代表 0.5 uV rms 全量程
    elif 3000<=M_freq_list[ii]<50000:
        sr860.write("SCAL 20")  # 设置灵敏度范围，20 代表 0.2 uV rms 全量程
    else:
        sr860.write("SCAL 21")  # 设置灵敏度范围，21 代表 0.1 uV rms 全量程
...
#保存测试结果
np.save(r"C:\Users\mantoutou\OneDrive\文档\中科大\超导量子计算科研\实验\Z板子噪声室温测试\锁相测试数据\Hefei_F111_6+\M_data_F111_6+_1e4_2.npy", M_dict_lists)
print("save .npy done")
# 关闭锁相放大器连接
sr860.close()
#%% 数据处理
...
#读取数据
Data_all = np.load(r"C:\Users\mantoutou\OneDrive\文档\中科大\超导量子计算科研\实验\Z板子噪声室温测试\锁相测试数据\Hefei_F111_6+\M_data_F111_6+_2e4.npy",
...
axes.plot(Freq_list,PSD_list,label = 'PSD of ZPA by SR860')
